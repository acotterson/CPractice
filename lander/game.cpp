...
void Game :: draw(const Interface & ui)
{
   ...
   Point xVelocityLocation;
   xVelocityLocation.setX(topLeft.getX() + 5);
   xVelocityLocation.setY(topLeft.getY() - 20);

   drawNumber(fuelLocation, lander.getFuel());
   drawNumber(xVelocityLocation, 10 * lander.getVelocity().getDx());

   Point yVelocityLocation;
   yVelocityLocation.setX(topLeft.getX() + 5);
   yVelocityLocation.setY(topLeft.getY() - 35);
 
   drawNumber(yVelocityLocation, 10 * lander.getVelocity().getDy());

   Point yLocationLocation;
   yLocationLocation.setX(topLeft.getX() + 5);
   yLocationLocation.setY(topLeft.getY() - 50);

   drawNumber(yLocationLocation, lander.getPoint().getY() + 200);

   if (lander.getPoint().getY() > 4500)
	   lander.setFuel(3);
   
   if (lander.getPoint().getY() >= 384400000)
   {
	   drawText(Point(), "You have reached Earth!");
	   lander.setFuel(0);
   }
